...
class backboneDiscriminator(nn.Module):  # D_f
    ...
    def forward(self, x):
        ...
        if x.size(0) == 1: #解决 Batch Normalization 在 batch size 为 1 时的技术问题
...
if __name__ == "__main__":
    ...
    seq_len = 70
    net = mymodel(d_model=24, max_len=seq_len).to('cuda')
    D1 = Discriminator(24).to('cuda')
    D2 = backboneDiscriminator(seq_len,24).to('cuda')
    Loss = nn.MSELoss()
    x = torch.randn(2, seq_len, 24).to('cuda')
    y = torch.randn(2, seq_len, 1).to('cuda')
    # opt = torch.optim.SGD(itertools.chain(net.parameters(), D1.parameters(), D2.parameters()), lr=)
    features, out = net(x, None)
    out.squeeze_(2)
    # domain1 = D1(out)
    domain2 = D2(features) #(batch, 1)
    pass