...
#全连接层 (nn.Linear)：默认使用Kaiming均匀分布初始化（He初始化），并且偏置项根据输入特征数量计算一个适当的边界值进行均匀分布初始化。
    ...
    def forward(self, source, target, constant=1):
        ...
        return (prediction, self.discriminator(source_feature), self.discriminator(target_feature),
            torch.torch.flatten(source_feature, start_dim=1),torch.torch.flatten(target_feature, start_dim=1))
...
class NEW_LSTM(nn.Module):
    def __init__(self, input_dim=18):
        ...
        # 第一层 BLSTM：64节点，Dropout 0.2 [cite: 270]
        self.blstm1 = nn.LSTM(input_size=input_dim,
                              hidden_size=64,
                              batch_first=True,
                              bidirectional=True)
        self.dropout1 = nn.Dropout(0.2)

        # 第二层 BLSTM：64节点，Dropout 0.2 [cite: 270]
        self.blstm2 = nn.LSTM(input_size=64 * 2,
                              hidden_size=64,
                              batch_first=True,
                              bidirectional=True)
        self.dropout2 = nn.Dropout(0.2)
        self.predictor = nn.Sequential(
            # 第三层：Flatten (在 forward 中通过 reshape 实现)
            nn.Flatten(),
            # 第四层：全连接层 (Dense Layer)
            # 输入维度计算：30(时序长度) * 64(隐藏节点) * 2(双向) = 3840
            nn.Linear(30 * 64 * 2, 128),
            nn.Dropout(0.5),
            # 输出层
            nn.Linear(128, 1)
        )
    ...
    def forward(self, source, target, target_feature, mode, constant=1):
            source, _ = self.blstm1(source)
            source = self.dropout1(source)

            # 第二层 LSTM 提取 output
            source, _ = self.blstm2(source)
            source_feature = self.dropout2(source) # (b,30,18)->>(b,30,128)
            prediction = self.predictor(source_feature)
            if mode == "mode1":
                target, _ = self.blstm1(target)
                target = self.dropout1(target)

                # 第二层 LSTM 提取 output
                target, _ = self.blstm2(target)
                target_feature = self.dropout2(target)  # (b,30,18)->>(b,30,128)
                target_feature = GradReverse.grad_reverse(target_feature, constant)
            elif mode == "mode2":
                pass
            # source_feature = torch.mean(source_feature, dim=2)#特征维度平均#(b,30,18)->>(b,30)
