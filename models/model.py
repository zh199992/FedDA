...
class GHDR_FL_testeds(nn.Module):
    ...
    def forward(self, x):
        ...
        shallow=self.F(x)
        # x=self.LHDR(x)
        x=torch.cat((self.LHDR(shallow), self.E_DS(shallow)), dim=-1)
        x=x.squeeze(1)
        x=self.unique(x)
        return x, shallow

class Cloud_GHDR(nn.Module):
    def __init__(self,input_size, window_size, conv_init='kaiming_uniform',  linear_init='xavier_uniform'):
        super(Cloud_GHDR, self).__init__()
        self.client_number=4
        self.input_size=input_size
        self.filter_num=10
        self.filter_length=10
        ...
        self.discriminator = nn.Sequential(
            # nn.Flatten(),
            nn.Linear(input_size*window_size, 64),
            nn.ReLU(),
            nn.Linear(64, 32),
            nn.ReLU(),
