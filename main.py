...
from system.server.serverStrongDA import serverStrongDA
...
def run(args):
    ...
    if args.algorithm == "GHDR":
        ...
        # elif model_str == "cnn2D":
            ...
            raise NotImplementedError
        if args.P_FedAvg == False and args.F_FedAvg == True and args.EDI_FedAvg == True:
            pass
        else:
            raise ValueError("not fedper")
        server = serverAvg(args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-algo', "--algorithm", type=str, default="FedPer",##这个参数不传入server
                        choices=["centralized", "local", "localiid", "FedAvg", "FedPer","FedAvgiid", "GHDR","FedDA","ablation1","ablation2", "finetune", "DANN", "SAWA"])
    parser.add_argument('-enable_cloud_da', "--enable_cloud_da", type=bool, default=False)
    parser.add_argument('-new_da', "--new_da", type=bool, default=False)
    parser.add_argument('-enable_CADA', "--enable_CADA", type=bool, default=False)
    ...
    parser.add_argument('-linear_init', "--linear_init", type=str, default="kaiming_uniform",
                        choices=["kaiming_uniform","kaiming_normal","xavier_uniform","xavier_normal","normal","uniform"])
    parser.add_argument('-F_FedAvg', "--F_FedAvg", type=bool, default=True)
    parser.add_argument('-EDI_FedAvg', "--EDI_FedAvg", type=bool,default=True)#不freeze也不fedavg就是个性化
