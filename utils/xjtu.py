...
# 假设CSV文件路径
folder_path = r"/home/zhouheng/project/FedDA/data/XJTU-SY_Bearing_Datasets/"  # 替换成你文件所在的目录
CONDITION_DATA_FOLDER_LIST = ["35Hz12kN", "37.5Hz11kN", "40Hz10kN"]

num_files = [[123, 161, 158, 122, 52], [491, 161, 533, 42, 339], [2538, 2496, 371, 1515, 114]]
...
for i in range(1, num_files + 1):
    ...
    # 保证只取前两列（防止空列或额外列）
    # data = df.iloc[:, :2].values  # shape: (32768, 2)
    data = df.iloc[start:end, :2].values  # shape: (20000, 2)
    for col in range(2):
...
# 转为 NumPy 数组: (100, 32768, 2)
all_data_np = np.stack(all_data)

# reshape 为 (10000, 32768, 2)
# 注意：100 * 1 = 10000 说明每个文件数据按 batch size 1 处理
tensor_data = torch.tensor(all_data_np, dtype=torch.float32).reshape(10000, signal_length, channels)
...
# 得到3个(5,sample_num,20000,2)的tensor 还是15个(sample_num,20000,2)的tensor，label就是用csv数量算rul吗。
XJTU_DATA_PATH = r"/home/zhouheng/project/FedDA/data/XJTU-SY_Bearing_Datasets/"
