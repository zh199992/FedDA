...
class clientCADA(Client):
    def __init__(self, args, id, train_samples, test_samples, **kwargs):
        ...
        self.optimizer = torch.optim.Adam([
            {'params': self.model.F.parameters(), 'lr': 5e-5},
            {'params': self.model.LHDR.parameters(), 'lr': 5e-5},
            {'params': self.model.predictor.parameters(), 'lr': 5e-5},
    ...
    def train(self):
        ...
        for epoch in range(max_local_epochs):
            ...
            for i, (x, y) in enumerate(self.trainloader):
                ...
                #     print("enable_CADA")
                    loss += self.args.info_lambda*infonce_loss  #为什么y1pjwvet只用lambda也能跑通？
    ...
    def set_parameters(self, model):
        ...
        if self.args.EDI_FedAvg:
            for new_param, old_param in zip(model.LHDR_list[self.id-1].parameters(), self.model.LHDR.parameters()):#可以选择
                old_param.data = new_param.data.clone()
